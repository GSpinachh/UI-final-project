...
class ProgressBar(BasePage):
    ...
    @allure.step('Дождаться заполнения шкалы на 75%')
    ...
    def get_result_value(self) -> int:
        ...
        return int(number_str[0]) if number_str else 0

    @allure.step('Проверить, что значение "Result" меньше чем 5')
    ...
    @allure.step('Проверить, что значение "Duration" меньше чем 17000')
